<template>
  <c-menu position="bottom" offset="-8px" :close-on-content-click="false" :max-height="310">
    <template #activator="{ props: activatorProps }">
      <c-input-container
        class="select-field"
        v-bind="{ ...activatorProps, ...inputContainerProps, ...inputContainerContext.emitEvents }"
      >
    ...
    <c-list>
      <c-list-item v-for="item in options" :key="item.value" :title="item.label"></c-list-item>
    </c-list>
  </c-menu>
</template>
...
interface InputSelectProps extends InputControlProps, InputContainerProps, CanDebounceInputProps {
